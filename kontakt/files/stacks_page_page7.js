...
stacks.stacks_in_1168 = {};
stacks.stacks_in_1168 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;

return stack;})(stacks.stacks_in_1168);
stacks.stacks_in_1192 = {};
stacks.stacks_in_1192 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; const form=$("#stacks_in_1192");if(form.hasClass("auto-submit-on-select-change")){$("select",form).change(()=>form.submit());}
...
$.ajax({type:"POST",url:"files/"+stackid+"_recaptcha.php",data:formdata,cache:false,async:false,contentType:false,processData:false,success:(data)=>{if(typeof callback==="function"){callback();}},error:(data)=>{console.error("Error checking reCAPTCHA");console.error(data);$(".g-recaptcha iframe",form).addClass("error");form_error();},complete:()=>{grecaptcha.reset();}});};stacks.forms.stacks_in_1192={};stacks.forms.stacks_in_1192.ajax_submit=(postUrl,postType="POST",jsonData=false)=>{if(typeof grecaptcha!=="undefined"){form_recaptcha(function(){form_submit(postUrl,postType,jsonData);});}else if($(".topyenoh input").length>0){if($(".topyenoh input").val().length==0){form_submit(postUrl,postType,jsonData);}else{console.warn("Failed honeypot. Release the honey badgers!");}}else{form_submit(postUrl,postType,jsonData);}};

return stack;})(stacks.stacks_in_1192);
stacks.stacks_in_1224 = {};
stacks.stacks_in_1224 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; const element=document.getElementById("f6-action-stacks_in_1224");if(element==null)return;$(document).on("formvalid.zf.abide",function(e,form){if(element.parentNode.id==form[0].id){console.log(`Triggering Email Action for form id"${form[0].id}"`);stacks.forms[element.parentNode.id].ajax_submit("files/stacks_in_1224_mailer.php");}});
