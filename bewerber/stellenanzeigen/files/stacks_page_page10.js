...
stacks.stacks_in_1347_11 = {};
stacks.stacks_in_1347_11 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;
$(function(){$(window).on('load',function(){$('#stacks_in_1347_11>.s-pro:not(.vault-height)>.shear-wrapper').removeClass('preload-wrapper');$('#stacks_in_1347_11>.s-pro:not(.vault-height) .content').removeClass('preload-content');});});

return stack;})(stacks.stacks_in_1347_11);
stacks.stacks_in_1347_9 = {};
stacks.stacks_in_1347_9 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;
$(function(){var win=$(window),stack=$('#stacks_in_1347_9'),spacer=$('.spacer_stacks_in_1347_9'),positionClass='section-bottom',sideClass='section-full-width',sectionWrapperId='#'+stack.closest('.shear-wrapper').attr('id'),sectionContent=stack.closest('.content-wrapper');stack.unwrap();stack.addClass('section-absolute').addClass(positionClass).addClass(sideClass).addClass('').appendTo(sectionWrapperId);if(positionClass=='section-bottom'){spacer.appendTo(sectionContent);}else{spacer.prependTo(sectionContent);}var resizeSpacer=function(){var targetHeight=stack.outerHeight();spacer.css('height',targetHeight);}
win.on("resize",resizeSpacer);resizeSpacer();$(window).load(function(){$('.section-fix',stack).removeClass('pre-load');resizeSpacer();});});

return stack;})(stacks.stacks_in_1347_9);
stacks.stacks_in_1347_37 = {};
stacks.stacks_in_1347_37 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; const form=$("#stacks_in_1347_37");if(form.hasClass("auto-submit-on-select-change")){$("select",form).change(()=>form.submit());}
...
$.ajax({type:"POST",url:"files/"+stackid+"_recaptcha.php",data:formdata,cache:false,async:false,contentType:false,processData:false,success:(data)=>{if(typeof callback==="function"){callback();}},error:(data)=>{console.error("Error checking reCAPTCHA");console.error(data);$(".g-recaptcha iframe",form).addClass("error");form_error();},complete:()=>{grecaptcha.reset();}});};stacks.forms.stacks_in_1347_37={};stacks.forms.stacks_in_1347_37.ajax_submit=(postUrl,postType="POST",jsonData=false)=>{if(typeof grecaptcha!=="undefined"){form_recaptcha(function(){form_submit(postUrl,postType,jsonData);});}else if($(".topyenoh input").length>0){if($(".topyenoh input").val().length==0){form_submit(postUrl,postType,jsonData);}else{console.warn("Failed honeypot. Release the honey badgers!");}}else{form_submit(postUrl,postType,jsonData);}};

return stack;})(stacks.stacks_in_1347_37);
stacks.stacks_in_1347_72 = {};
stacks.stacks_in_1347_72 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; const element=document.getElementById("f6-action-stacks_in_1347_72");if(element==null)return;$(document).on("formvalid.zf.abide",function(e,form){if(element.parentNode.id==form[0].id){console.log(`Triggering Email Action for form id"${form[0].id}"`);stacks.forms[element.parentNode.id].ajax_submit("files/stacks_in_1347_72_mailer.php");}});

return stack;})(stacks.stacks_in_1347_72);