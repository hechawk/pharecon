...
stacks.stacks_in_1303 = {};
stacks.stacks_in_1303 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; if(typeof(stacks.totalcms)!=='object')stacks.totalcms={};stacks.totalcms.requestheaders={'Total-Key':$.trim('6876b067a4663c90a3efcaee9ec3337a')};stacks.totalcms.baseurl='https://www.pharecon.de//'.replace(/\/\/$/,'/');stacks.totalcms.baseurl=stacks.totalcms.baseurl.replace('https:','').replace('http:','');stacks.totalcms.totalapi=stacks.totalcms.baseurl+'rw_common/plugins/stacks/total-cms/totalapi.php';stacks.totalcms.locale='de';

return stack;})(stacks.stacks_in_1303);
...
stacks.stacks_in_1389_70 = {};
stacks.stacks_in_1389_70 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;

return stack;})(stacks.stacks_in_1389_70);
stacks.stacks_in_1389_11 = {};
stacks.stacks_in_1389_11 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;
$(function(){var slideUp={cssHeadSlide:function(hide){if(!isHeadSlide||slideHeadSuspend)return false;var headDirection=relTop?'-':'',slideHeight=$('#stacks_in_1389_11>.chroma-scroll>.slide-head').outerHeight();if(hide){stack.css({'-webkit-transform':'translate(0,'+headDirection+slideHeight+'px)','-moz-transform':'translate(0,'+headDirection+slideHeight+'px)','-ms-transform':'translate(0,'+headDirection+slideHeight+'px)','-o-transform':'translate(0,'+headDirection+slideHeight+'px)','transform':'translate(0,'+headDirection+slideHeight+'px)'});}else{stack.css({'-webkit-transform':'translate(0,0)','-moz-transform':'translate(0,0)','-ms-transform':'translate(0,0)','-o-transform':'translate(0,0)','transform':'translate(0,0)'});}},closePopDrops:function(){try{popTarget.popDrop('hide');}
...
utils.updateTopBarState();facetBar=stack.find('.oc-toggle.facet-mb'),hasFacetBar=facetBar.length>0?true:false,utils.prepFacetBar();popDrop=stack.find('[data-pop-drop]').length>0?true:false;popTarget=$('[data-pop-drop]',stack);bpDropdown=stack.find('.bp-dropdown').length>0?true:false;dropdowns=$('.bp-dropdown');if(isHeadSlide){if(!relTop){slideHead.detach().insertAfter(suContent)};slideHead.addClass('');}};if(!document.getElementById("stacks_in_1389_11")){console.log("Choma removed by 3rd party script");return false;}
var lastRequestAnimationFrame=null,stack=$('#stacks_in_1389_11>.chroma-scroll'),wrapper=$('#stacks_in_1389_11'),spacer=$('.spacer-stacks_in_1389_11'),slideHead=$('.slide-head',stack),slideHeadSuspend=false,suContent=$('.su-content',stack),slideBP=''!=''?645:0,relTop='True'=='True',slideTime=600,initDelay=430>=430?430:430,isStatic='posFixed'=='posStatic'?true:false,isSticky='posFixed'=='posSticky'?true:false,didScroll,lastPosition=-999,lastPos=0,spacerSize=0,downTolerance=300,upTolerance=100,topOffset=0,offsetUnits='px',smallOffsetUnits=false===true?'px':'px',breakpoint=645,smallTopOffset=false===true?0:0,smallTopOffset=false===true?-9999:smallTopOffset,triggerTop=stack.offset().top-topOffset,fixToParentSize='mob-full-width'!='fix-full-width'?true:false,parentSizeBP='mob-full-width'=='mob-full-width'?breakpoint:0,fullWidth="width-scale"=="width-contain"&&"posFixed"=="posFixed"?false:true,slideType='scrollAnimate',isSlide=''!==''?true:false,isHeadSlide='ani-slide'=='header-slide'&&isSlide?true:false,markerSlide=false,upClass='slide-up',downClass='slide-down',facetBar={},hasFacetBar,popDrop=false,bpDropdown=false,popTarget=stack,dropdowns=stack,touchScreen=utils.isTouchDevice(),cancel_above_below=''=='below-bp'?1:''=='above-bp'?-1:0,cancel_breakpoint=cancel_above_below!==0?breakpoint:cancel_above_below;var resizeTimer;$(window).on('resize',function(e){clearTimeout(resizeTimer);resizeTimer=setTimeout(function(){utils.resizeElement();utils.updateTopBarState();utils.toggleSpacer(true);},10);});$(window).on('scroll',function(){markerScroll.updateScrollClasses(markerScroll.findMarker());didScroll=true});setInterval(function(){if(didScroll){didScroll=false;}},10);$(window).on('load',function(){if(!isStatic){init();}else{stack.addClass('is-static');}
...
return stack;})(stacks.stacks_in_1389_11);
stacks.stacks_in_69 = {};
...
var jQuery = stacks.jQuery;var $ = jQuery; $(function(){var stack=$('#blog_stacks_in_1315');if(stack.length===0)return;var processCards=function(){return false;},processDates=function(dates){dates.each(function(){var datetime=$(this).attr('datetime'),dateformat=$(this).data('dateformat');if(dateformat==='relative'){$(this).html(moment(datetime).locale(stacks.totalcms.locale).fromNow());}
...
var jQuery = stacks.jQuery;var $ = jQuery; $(document).ready(function(){var form=$("form.stacks_in_1342");$("button[type=reset]",form).click(function(){$("select",form).each(function(){$(this).find("option").attr("selected",false);$(this).find("option").first().attr("selected",true);});});if(form.hasClass("autosubmit")){$("select",form).change(()=>form.submit());}});
...
var jQuery = stacks.jQuery;var $ = jQuery;// Start dooFaq2 stack each Javascript code// iconate framework!function(){"use strict";function a(a,b,c){a.style.setProperty("-webkit-animation",b+" "+c+"s"),a.style.setProperty("animation",b+" "+c+"s"),a.style.setProperty("-moz-animation",b+" "+c+"s"),a.style.setProperty("-o-animation",b+" "+c+"s")}function b(a){a.style.removeProperty("-webkit-animation"),a.style.removeProperty("animation"),a.style.removeProperty("-moz-animation"),a.style.removeProperty("-o-animation")}function c(a,b,c){a.classList.contains(b)&&a.classList.remove(b),a.classList.add(c)}function d(d,l,m){function n(){var a=k(),b=p/2;q=window.setTimeout(function(){var e=k()-a;e>=b&&c(d,l.from,l.to)},b)}function o(){window.clearTimeout(q),b(d),d.removeEventListener(f,o),d.removeEventListener(e,n),"function"==typeof m&&m()}if(!d)throw new Error('Iconate > "element" is required');if(!j)return c(d,l.from,l.to),void("function"==typeof m&&m());l=l||{};var p,q,r;r=l.animation||i,p=l.duration||g,d.addEventListener(e,n,!1),d.addEventListener(f,o,!1),a(d,r,p/h)}var e,f,g=600,h=1e3,i="zoomOut",j=function(){var a=document.documentElement.style;return void 0!==a.animation||void 0!==a.webkitAnimation}();f=void 0===window.onanimationend&&void 0!==window.onwebkitanimationend?"webkitAnimationEnd":"animationend",e=void 0===window.onanimationstart&&void 0!==window.onwebkitanimationstart?"webkitAnimationStart":"animationstart";var k=Date.now||function(){return(new Date).getTime()};"object"==typeof exports?module.exports=d:"function"==typeof define&&define.amd?define(function(){return d}):window.iconate=d}();// faq code$(document).ready(function() {		// animate the icon	function stacks_in_1259animate(element,iconFrom,iconTo,animationType) {		stacks_in_1259IsAnimating = true;	    iconate(element, {	        from: iconFrom,	        to: iconTo,	        animation: animationType	    }, function() {		    	        stacks_in_1259callback();	    });	}		// toggle animation done	var stacks_in_1259callback = function() {		stacks_in_1259IsAnimating = false;	}		// add an icon to each if required	$("#stacks_in_1259 .faq-question").each(function(){		$(this).find("i").addClass('fa-bars');	});					// declare open vars	var stacks_in_1259IsAnimating = false;	switch("fa-bars") {	    case "fa-toggle-off":	        var stacks_in_1259iconOpenClass = "fa-toggle-on";			var stacks_in_1259iconClosedClass = "fa-toggle-off";			var stacks_in_1259iconAnimation = "bounceOutRight";			var stacks_in_1259iconCloseAnimation = "bounceOutLeft";	        break;	    case "fa-question":	        var stacks_in_1259iconOpenClass = "fa-times";			var stacks_in_1259iconClosedClass = "fa-question";			var stacks_in_1259iconAnimation = "tada";			var stacks_in_1259iconCloseAnimation = "rubberBand";	        break;	    case "fa-angle-right":	        var stacks_in_1259iconOpenClass = "fa-angle-down";			var stacks_in_1259iconClosedClass = "fa-angle-right";			var stacks_in_1259iconAnimation = "bounceOutBottom";			var stacks_in_1259iconCloseAnimation = "fadeOutLeft";	        break;	    case "fa-bars":	        var stacks_in_1259iconOpenClass = "fa-times";			var stacks_in_1259iconClosedClass = "fa-bars";			var stacks_in_1259iconAnimation = "tada";			var stacks_in_1259iconCloseAnimation = "rubberBand";	        break;	    default:	        var stacks_in_1259iconOpenClass = "fa-toggle-on";			var stacks_in_1259iconClosedClass = "fa-toggle-off";			var stacks_in_1259iconAnimation = "bounceOutRight";			var stacks_in_1259iconCloseAnimation = "bounceOutLeft";	}				// animation handlers	function stacks_in_1259open_faq_section(sectionQuestion) {		if("yes" == "yes"){			stacks_in_1259close_other_open_faq_section(sectionQuestion)		}		$(sectionQuestion).siblings(".faq-answer").slideDown(300,"swing");		iconElementId = $("i", sectionQuestion).attr("id");		var iconElement = document.getElementById(iconElementId);		stacks_in_1259animate(iconElement,stacks_in_1259iconClosedClass,stacks_in_1259iconOpenClass,stacks_in_1259iconAnimation);        $(sectionQuestion).removeClass("faqQuestionClosed").addClass("faqQuestionOpen");    }    function stacks_in_1259close_faq_section(sectionQuestion) {	    $(sectionQuestion).siblings(".faq-answer").slideUp(300,"swing");	    iconElementId = $("i", sectionQuestion).attr("id");		var iconElement = document.getElementById(iconElementId);		stacks_in_1259animate(iconElement,stacks_in_1259iconOpenClass,stacks_in_1259iconClosedClass,stacks_in_1259iconCloseAnimation);        $(sectionQuestion).removeClass("faqQuestionOpen").addClass("faqQuestionClosed");    }    function stacks_in_1259close_other_open_faq_section(sectionQuestion) {	    if($("#stacks_in_1259 .faqQuestionOpen").length){		    $("#stacks_in_1259 .faqQuestionOpen").siblings(".faq-answer").slideUp(300,"swing");			var iconElementId = $("#stacks_in_1259 .faqQuestionOpen i").attr("id");			var iconElement = document.getElementById(iconElementId);			stacks_in_1259animate(iconElement,stacks_in_1259iconOpenClass,stacks_in_1259iconClosedClass,stacks_in_1259iconCloseAnimation);		    $("#stacks_in_1259 .faqQuestionOpen").removeClass("faqQuestionOpen").addClass("faqQuestionClosed");	    }    }	// on clicking a question	$('#stacks_in_1259 .faq-question').on("click", function(e){		e.preventDefault();		if (stacks_in_1259IsAnimating) return;				if($(this).hasClass("faqQuestionOpen")){			stacks_in_1259close_faq_section($(this));		}else{			stacks_in_1259open_faq_section($(this));		}	});		// strat with first item open or not	if("no" == "yes"){		var stacks_in_1259firstItem = $("#stacks_in_1259 .faq-question").get(0);		stacks_in_1259open_faq_section($(stacks_in_1259firstItem));	}}); // end doc ready// End dooFaq2 stack each Javascript code
return stack;})(stacks.stacks_in_1259);
stacks.stacks_in_1276 = {};
stacks.stacks_in_1276 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;// Start dooFaq2 stack each Javascript code// iconate framework!function(){"use strict";function a(a,b,c){a.style.setProperty("-webkit-animation",b+" "+c+"s"),a.style.setProperty("animation",b+" "+c+"s"),a.style.setProperty("-moz-animation",b+" "+c+"s"),a.style.setProperty("-o-animation",b+" "+c+"s")}function b(a){a.style.removeProperty("-webkit-animation"),a.style.removeProperty("animation"),a.style.removeProperty("-moz-animation"),a.style.removeProperty("-o-animation")}function c(a,b,c){a.classList.contains(b)&&a.classList.remove(b),a.classList.add(c)}function d(d,l,m){function n(){var a=k(),b=p/2;q=window.setTimeout(function(){var e=k()-a;e>=b&&c(d,l.from,l.to)},b)}function o(){window.clearTimeout(q),b(d),d.removeEventListener(f,o),d.removeEventListener(e,n),"function"==typeof m&&m()}if(!d)throw new Error('Iconate > "element" is required');if(!j)return c(d,l.from,l.to),void("function"==typeof m&&m());l=l||{};var p,q,r;r=l.animation||i,p=l.duration||g,d.addEventListener(e,n,!1),d.addEventListener(f,o,!1),a(d,r,p/h)}var e,f,g=600,h=1e3,i="zoomOut",j=function(){var a=document.documentElement.style;return void 0!==a.animation||void 0!==a.webkitAnimation}();f=void 0===window.onanimationend&&void 0!==window.onwebkitanimationend?"webkitAnimationEnd":"animationend",e=void 0===window.onanimationstart&&void 0!==window.onwebkitanimationstart?"webkitAnimationStart":"animationstart";var k=Date.now||function(){return(new Date).getTime()};"object"==typeof exports?module.exports=d:"function"==typeof define&&define.amd?define(function(){return d}):window.iconate=d}();// faq code$(document).ready(function() {		// animate the icon	function stacks_in_1276animate(element,iconFrom,iconTo,animationType) {		stacks_in_1276IsAnimating = true;	    iconate(element, {	        from: iconFrom,	        to: iconTo,	        animation: animationType	    }, function() {		    	        stacks_in_1276callback();	    });	}		// toggle animation done	var stacks_in_1276callback = function() {		stacks_in_1276IsAnimating = false;	}		// add an icon to each if required	$("#stacks_in_1276 .faq-question").each(function(){		$(this).find("i").addClass('fa-bars');	});					// declare open vars	var stacks_in_1276IsAnimating = false;	switch("fa-bars") {	    case "fa-toggle-off":	        var stacks_in_1276iconOpenClass = "fa-toggle-on";			var stacks_in_1276iconClosedClass = "fa-toggle-off";			var stacks_in_1276iconAnimation = "bounceOutRight";			var stacks_in_1276iconCloseAnimation = "bounceOutLeft";	        break;	    case "fa-question":	        var stacks_in_1276iconOpenClass = "fa-times";			var stacks_in_1276iconClosedClass = "fa-question";			var stacks_in_1276iconAnimation = "tada";			var stacks_in_1276iconCloseAnimation = "rubberBand";	        break;	    case "fa-angle-right":	        var stacks_in_1276iconOpenClass = "fa-angle-down";			var stacks_in_1276iconClosedClass = "fa-angle-right";			var stacks_in_1276iconAnimation = "bounceOutBottom";			var stacks_in_1276iconCloseAnimation = "fadeOutLeft";	        break;	    case "fa-bars":	        var stacks_in_1276iconOpenClass = "fa-times";			var stacks_in_1276iconClosedClass = "fa-bars";			var stacks_in_1276iconAnimation = "tada";			var stacks_in_1276iconCloseAnimation = "rubberBand";	        break;	    default:	        var stacks_in_1276iconOpenClass = "fa-toggle-on";			var stacks_in_1276iconClosedClass = "fa-toggle-off";			var stacks_in_1276iconAnimation = "bounceOutRight";			var stacks_in_1276iconCloseAnimation = "bounceOutLeft";	}				// animation handlers	function stacks_in_1276open_faq_section(sectionQuestion) {		if("yes" == "yes"){			stacks_in_1276close_other_open_faq_section(sectionQuestion)		}		$(sectionQuestion).siblings(".faq-answer").slideDown(300,"swing");		iconElementId = $("i", sectionQuestion).attr("id");		var iconElement = document.getElementById(iconElementId);		stacks_in_1276animate(iconElement,stacks_in_1276iconClosedClass,stacks_in_1276iconOpenClass,stacks_in_1276iconAnimation);        $(sectionQuestion).removeClass("faqQuestionClosed").addClass("faqQuestionOpen");    }    function stacks_in_1276close_faq_section(sectionQuestion) {	    $(sectionQuestion).siblings(".faq-answer").slideUp(300,"swing");	    iconElementId = $("i", sectionQuestion).attr("id");		var iconElement = document.getElementById(iconElementId);		stacks_in_1276animate(iconElement,stacks_in_1276iconOpenClass,stacks_in_1276iconClosedClass,stacks_in_1276iconCloseAnimation);        $(sectionQuestion).removeClass("faqQuestionOpen").addClass("faqQuestionClosed");    }    function stacks_in_1276close_other_open_faq_section(sectionQuestion) {	    if($("#stacks_in_1276 .faqQuestionOpen").length){		    $("#stacks_in_1276 .faqQuestionOpen").siblings(".faq-answer").slideUp(300,"swing");			var iconElementId = $("#stacks_in_1276 .faqQuestionOpen i").attr("id");			var iconElement = document.getElementById(iconElementId);			stacks_in_1276animate(iconElement,stacks_in_1276iconOpenClass,stacks_in_1276iconClosedClass,stacks_in_1276iconCloseAnimation);		    $("#stacks_in_1276 .faqQuestionOpen").removeClass("faqQuestionOpen").addClass("faqQuestionClosed");	    }    }	// on clicking a question	$('#stacks_in_1276 .faq-question').on("click", function(e){		e.preventDefault();		if (stacks_in_1276IsAnimating) return;				if($(this).hasClass("faqQuestionOpen")){			stacks_in_1276close_faq_section($(this));		}else{			stacks_in_1276open_faq_section($(this));		}	});		// strat with first item open or not	if("no" == "yes"){		var stacks_in_1276firstItem = $("#stacks_in_1276 .faq-question").get(0);		stacks_in_1276open_faq_section($(stacks_in_1276firstItem));	}}); // end doc ready// End dooFaq2 stack each Javascript code
return stack;})(stacks.stacks_in_1276);
stacks.stacks_in_1399_11 = {};
stacks.stacks_in_1399_11 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;
$(function(){$(window).on('load',function(){$('#stacks_in_1399_11>.s-pro:not(.vault-height)>.shear-wrapper').removeClass('preload-wrapper');$('#stacks_in_1399_11>.s-pro:not(.vault-height) .content').removeClass('preload-content');});});

return stack;})(stacks.stacks_in_1399_11);
stacks.stacks_in_1399_9 = {};
stacks.stacks_in_1399_9 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery;
$(function(){var win=$(window),stack=$('#stacks_in_1399_9'),spacer=$('.spacer_stacks_in_1399_9'),positionClass='section-bottom',sideClass='section-full-width',sectionWrapperId='#'+stack.closest('.shear-wrapper').attr('id'),sectionContent=stack.closest('.content-wrapper');stack.unwrap();stack.addClass('section-absolute').addClass(positionClass).addClass(sideClass).addClass('').appendTo(sectionWrapperId);if(positionClass=='section-bottom'){spacer.appendTo(sectionContent);}else{spacer.prependTo(sectionContent);}var resizeSpacer=function(){var targetHeight=stack.outerHeight();spacer.css('height',targetHeight);}
win.on("resize",resizeSpacer);resizeSpacer();$(window).load(function(){$('.section-fix',stack).removeClass('pre-load');resizeSpacer();});});

return stack;})(stacks.stacks_in_1399_9);
stacks.stacks_in_1399_37 = {};
stacks.stacks_in_1399_37 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; const form=$("#stacks_in_1399_37");if(form.hasClass("auto-submit-on-select-change")){$("select",form).change(()=>form.submit());}
...
$.ajax({type:"POST",url:"files/"+stackid+"_recaptcha.php",data:formdata,cache:false,async:false,contentType:false,processData:false,success:(data)=>{if(typeof callback==="function"){callback();}},error:(data)=>{console.error("Error checking reCAPTCHA");console.error(data);$(".g-recaptcha iframe",form).addClass("error");form_error();},complete:()=>{grecaptcha.reset();}});};stacks.forms.stacks_in_1399_37={};stacks.forms.stacks_in_1399_37.ajax_submit=(postUrl,postType="POST",jsonData=false)=>{if(typeof grecaptcha!=="undefined"){form_recaptcha(function(){form_submit(postUrl,postType,jsonData);});}else if($(".topyenoh input").length>0){if($(".topyenoh input").val().length==0){form_submit(postUrl,postType,jsonData);}else{console.warn("Failed honeypot. Release the honey badgers!");}}else{form_submit(postUrl,postType,jsonData);}};

return stack;})(stacks.stacks_in_1399_37);
stacks.stacks_in_1399_72 = {};
stacks.stacks_in_1399_72 = (function(stack) {
var jQuery = stacks.jQuery;var $ = jQuery; const element=document.getElementById("f6-action-stacks_in_1399_72");if(element==null)return;$(document).on("formvalid.zf.abide",function(e,form){if(element.parentNode.id==form[0].id){console.log(`Triggering Email Action for form id"${form[0].id}"`);stacks.forms[element.parentNode.id].ajax_submit("files/stacks_in_1399_72_mailer.php");}});

return stack;})(stacks.stacks_in_1399_72);